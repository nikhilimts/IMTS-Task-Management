...
const SystemAdminEmployees: React.FC = () => {
  ...
  const fetchEmployees = async () => {
    try {
      ...
      setPagination({
        ...
        totalItems: response.data.data.pagination.totalUsers,
  ...
  const handleAccessChange = async (userId: string, isActive: boolean) => {
    try {
      ...
      await adminService.toggleUserAccess(userId, isActive);
  ...
  const getRoleBadge = (role: string) => {
    ...
    return (
      <span className={`inline-flex items-center px-2 py-1 text-xs font-semibold rounded-full ${roleColors[role] || 'bg-gray-100 text-gray-800'}`}>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-gray-500">
                  System-wide employee management - {pagination.totalItems} employees
                </p>
              </div>
              ...
              <div key={employee._id} className="bg-white rounded-lg shadow-sm p-6 hover:shadow-md transition-shadow">
                ...
                <div className="mt-4 space-y-2">
                  <div className="flex items-center text-sm text-gray-600">
                    ...
                    <span className="ml-2">{getRoleBadge(employee.role)}</span>
                  </div>
                  <div className="flex items-center text-sm text-gray-600">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs text-gray-600">
                    <div>Total Tasks: {employee.taskStats.totalTasks}</div>
                    <div>Completed: {employee.taskStats.completedTasks}</div>
                  </div>
                </div>
